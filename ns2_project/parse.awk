...

{
    ...
    if (event == "N") {
        ...
        node =$5
        energy_value= $7
        energy_consumed[node] =25-energy_value;
    }
    else {
        time_sec = $2;
        node = $3;
...
END {
    ...
    for(i=0;i<100;i++) {
	total_energy_consumption = total_energy_consumption + energy_consumed[i];
}
...
#
    ...
    print (received_bytes * 8) / simulation_time "," (total_delay / received_packets) "," (received_packets / sent_packets) "," (dropped_packets / sent_packets) "," total_energy_consumption;
}